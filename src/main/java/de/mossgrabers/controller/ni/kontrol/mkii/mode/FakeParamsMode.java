 ...
 */
public class FakeParamsMode extends AbstractParameterMode<KontrolProtocolControlSurface, KontrolProtocolConfiguration, IParameter>
{
     ...
     */
    public FakeParamsMode (final KontrolProtocolControlSurface surface, final IModel model, final List<ContinuousID> controls)
    {
    ...
    public int getKnobValue (final int index)
    {
        ...
        {
            final IParameter parameter = this.bank.getItem (index - KontrolProtocolControlSurface.CC_TRACK_VOLUME);
        ...
        {
            final IParameter parameter = this.bank.getItem (index - KontrolProtocolControlSurface.CC_TRACK_PAN);
            return valueChanger.toMidiValue (parameter.getValue ());
        }
    ...
    public void updateDisplay ()
    {
        ...
        for (int i = 0; i < 8; i++)
        {
            ...
            // Track Available
            this.surface.sendKontrolSysEx (KontrolProtocolControlSurface.SYSEX_TRACK_AVAILABLE, TrackType.GENERIC, i);
            this.surface.sendKontrolSysEx (KontrolProtocolControlSurface.SYSEX_TRACK_SELECTED, parameter.isSelected () ? 1 : 0, i);
            ...
            this.surface.sendKontrolSysEx (KontrolProtocolControlSurface.SYSEX_TRACK_VOLUME_TEXT, 0, i, info);
            this.surface.sendKontrolSysEx (KontrolProtocolControlSurface.SYSEX_TRACK_PAN_TEXT, 0, i, info);
            final String name = this.getLabel (selectedPage, parameter);
            this.surface.sendKontrolSysEx (KontrolProtocolControlSurface.SYSEX_TRACK_NAME, 0, i, name);
    ...
    {
        this.activeProviderIndex = Math.clamp (index, 0, 2);
